...
@endphp
    ...
    <table>
        ...
        @foreach ($transferBanco as $b)
            <tr><td class="sub">· {{ mb_strtoupper($b['banco']) }}</td><td class="der chico">L. {{ number_format($b['total'], 2) }}</td></tr>
        @endforeach
        {{-- Consumos de cuentas prepago: comida que salió hoy pero que YA se
             cobró y se facturó el día del depósito. No es dinero de hoy y no
             entra al total de ventas de arriba — se muestra solo para que la
             caja sepa cuánta comida salió sin cobro en el turno. --}}
        @if ((float) $corte->total_saldo > 0)
            <tr><td>CONSUMO DE CUENTAS</td><td class="der">L. {{ number_format((float) $corte->total_saldo, 2) }}</td></tr>
            <tr><td colspan="2" class="chico">NO ES VENTA DE HOY: YA SE FACTURO AL DEPOSITAR.</td></tr>
        @endif
